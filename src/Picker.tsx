  ...
  return (
    <TouchListener ref={ref}
      ...
      <div>
        ...
        <StyledItem />
        {items.map((item, i) => (
          <StyledItem key={i} active={index === i && light} onClick={() => handleItemClick(i)}>
            {
              typeof item === 'object' ? (item[contentKey] as any) : (item as any)
            }
          </StyledItem>
        ))}
        <StyledItem />
...
export interface PickerProps<T = string> extends HTMLAttributes<HTMLDivElement> {
  /** 是否可见 */
  visible: boolean
  /** 标题 */
  title: string
