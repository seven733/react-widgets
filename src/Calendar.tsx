...
export default function Calendar(props: CalendarProps) {
  const now = Date.now()
  const {
    month: propsMonth = now, activeDates = [], range: [start, end] = [0, 0], today = now, children,
    header, onClickDate, onHoverDate, enabledDates, disabledDates, ...others } = props
...
interface CalendarProps extends HtmlHTMLAttributes<HTMLDivElement> {
  month?: number // 该月份的毫秒数
  today?: number // 今天的毫秒数
  activeDates?: number[] // 需要高亮的日期1...31
  range?: [number, number] // 需要渲染背景色的开始和结束日期, 如[5, 25]
  onHoverDate?: (date: number) => void // 悬停在哪个日期
  onClickDate?: (date: number) => void // 日期点击时间
  header?: ReactNode
